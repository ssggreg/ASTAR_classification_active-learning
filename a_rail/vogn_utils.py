...
def test_perf_py(Xtr,Ytr,Xte,Yte):
    ...
    final_loader = torch.utils.data.DataLoader(file_dataset,batch_size=4, shuffle=False)
...
def final_fast_multi_vogn(depart,nb_ech,nb_ep,nb_batch,batch_size_sample,vogn_batch_size,class_model,X,Y,Xtest,Ytest,seeds,ttt,f):
    ...
    inference_loader = torch.utils.data.DataLoader(inference_dataset,batch_size=9472, shuffle=False)
    ...
    optimizer, num_epochs=nb_ep[k])
        ...
        labz=torch.zeros(nb_ech,9472).cuda()
        predict = torch.zeros(nb_ech,9472).cuda()
        model.eval()
        with torch.no_grad():
            for i in range(nb_ech):
                predictions,lbl = inference_pp(model, inference_loader,optimizer,1)
                predict[i] = predictions.view(9472)
                labz[i] = lbl.view(9472)
