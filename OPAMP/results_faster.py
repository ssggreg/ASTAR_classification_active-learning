...
np.save('random_accuracies_1_100_step1_fast', tss)
np.save('VOGN_accuracies_1_81_step1_fast', ttt)

w = csv.writer(open("VOGN_selection_fast.csv", "w"))
for key, val in rez_.items():
    w.writerow([key, val])
    
w = csv.writer(open("Random_selection_fast.csv", "w"))
...
plot_mean_std((np.arange(684)+1)/3420,tss,'random selection_fast')
plot_mean_std(nnn/3420,ttt,'VOGN selection_fast',color='y')
plt.savefig('graph_1_fast.png')    
